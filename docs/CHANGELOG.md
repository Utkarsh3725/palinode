...
- **`palinode migrate frontmatter` — idempotent, non-destructive frontmatter backfill, and `daily/` is settled as a log tier (#309).** Fills `id` / `category` / `type` (and `created_at` / `last_updated`) on memories that predate required-field validation, deriving each value only from an honest source — the containing directory, the filename, a legacy field of identical meaning (`created` → `created_at`), or git history — and leaving a field *absent and reported* when no such source exists rather than inventing one (mtime is deliberately not treated as provenance). Existing values are never overwritten, so re-running is a no-op; dry-run is the default, and each applied file is written through the `git_tools` mutation choke point and committed on its own with every value's derivation recorded in the commit body. Alongside it, **`daily/` notes are now formally the structural log tier** — append-only, N sessions per file, with each session already persisted separately as a typed memory by session-end's dual-write — so they are exempt from the required-frontmatter contract (new PROGRAM.md § File tiers). `palinode lint` no longer reports them under `missing_fields`, closing an inconsistency where `daily/` violations were counted in a numerator whose denominator (`total_files`) already excluded them, nor under `wiki_drift`, which compared a session summary's `[[wikilinks]]` against an `entities:` list a log never carries; the backfill skips them for the same reason (`--daily-mode minimal` fills id/category/dates anyway, never a `type:`).
- **On-demand ARCHIVE / SUPERSEDE for one named memory (#664).** `palinode archive <file> [--reason …] [--superseded-by <slug>]`, `POST /archive`, and the `palinode_archive` MCP tool make the documented retirement path addressable: until now the only archive entrypoint was the bulk TTL sweep, so an agent that knew *this* memory was wrong hand-wrote a tombstone body — which never sets `status`, leaving the superseded content live in recall. One verb, one optional argument (`superseded_by` names the replacement and makes it a SUPERSEDE); both end at `status: archived`, never `superseded`, because only `archived` is in `config.search.exclude_status`. Reuses the shipped machinery rather than reimplementing it — the frontmatter flip is the primitive the TTL sweep now shares, the audit trail is the executor's own `{base}-history.md` writer (so `palinode trace` reads the result as a supersession trail unchanged), and the chunk-status push is `store.set_status_for_path`. Idempotent, path-validated, and one mutation = one commit staging the memory plus its history sibling. Registered in `palinode/core/parity.py` (plugin-exempt like `archive_expired` / `consolidate` / `rollback`); MCP schema + API route-inventory goldens regenerated. New `tests/test_archive_on_demand_664.py` (31 tests). Also reconciles the written spec with the shipped behaviour (#690): `PROGRAM.md`, `PRD.md`, `specs/prompts/update.md` and `specs/prompts/context-assembly.md` all instructed marking a retired memory `status: superseded` and claimed `config.search.exclude_status` suppressed it — it does not, and nothing anywhere filters on `superseded`, so anything written to spec stayed fully recallable. They now prescribe `status: archived` plus `superseded_by`, matching what the executor and the TTL sweep have always done.

### Changed

### Fixed

- **`palinode repair-status --execute` now repoints the index itself (#679 follow-up).** The command told operators to run `palinode reindex` afterwards "so the entity graph is rebuilt from the corrected frontmatter" — in three places. That does not work, and shipping the instruction would have left every operator's repair half-applied with the tool reporting success: the repair is a frontmatter-only change, so it never moves the body content-hash `index_file` keys its fast path on, and a re-index reports every repaired file unchanged (#698). New `store.set_entities_for_path`, a sibling of the existing `set_status_for_path` (#482) written for the same reason, updates `chunks.metadata['entities']` **and** replaces the file's rows in the `entities` table — the delete matters, because `upsert_entities` only ever inserts, so a corrected ref adds a row and orphans the old one (#699). Both run in one transaction. Best-effort: a missing or unreadable index never fails the file repair, since the file on disk is the source of truth and is already correct. The three instruction sites now say what actually happens.
- **Consolidation status-file integrity (#679, #470).** `projects/*-status.md` — the file consolidation writes about itself — is auditable again. The write path now reads operation fields through the same `op_kind()`/`op_reason()` accessors the `--dry-run` preview uses (so a rationale-first ARCHIVE/RETRACT logs its rationale instead of blank, and a kind-less op logs as `KEEP`, matching the executor, instead of falsely claiming `UPDATE`), suppresses no-op `KEEP` lines, replaces any `fact_id` that matches no `<!-- fact:… -->` marker in the target with `(unresolved)` rather than writing LLM free text verbatim, bounds the Consolidation Log to the 10 most recent date blocks (`consolidation.status_log_max_blocks`) plus a cumulative elision line, and reconciles `memory_count`/`date_range`/`last_updated` with the body on every write. Root cause of the malformed `entities:` frontmatter was separate and is also fixed: fact-ID tagging and the executor both matched `- item` list syntax across the *whole file*, so YAML frontmatter entries were tagged as facts and later rewritten with status prose; both are now confined to the document body. `palinode repair-status` (dry-run by default) repairs existing files — `--scope all` additionally strips frontmatter markers across `people/`, `projects/`, `decisions/` and `insights/`, the four directories `bootstrap-fact-ids` walks, because a marker embedded in an entity ref's string value forks it into its own entity-graph node.
- **Deterministic background-thread shutdown, and guards that make a green test run mean something (#677, #356).** `PalinodeHandler` gained `shutdown()`, and the watcher registers an `atexit` hook that cancels and joins every live handler's summary/description debounce timers — so no daemon `threading.Timer` survives interpreter finalization, which is what made CI abort with exit 134 (`_enter_buffered_busy` on stderr) *after* every test had already passed; `palinode-watcher` also stops its timers on Ctrl-C instead of leaving one armed, and an over-budget `doctor` check no longer logs once the runner has abandoned it. On the test-signal side: `tests/test_collection_integrity.py` asserts every test module is collected and yields at least the items it declares, so a silently uncollected module — the worry behind #356, whose reported 134-test "gap" was a unit-only vs unit+integration count mismatch — fails by name instead of shrinking the count; the suite now also restores the global `config` singleton after every test (a failing test used to leak `memory_dir` / `git.auto_commit` into every test after it) and pins colour-forcing environment variables off, so a local run matches CI instead of failing 8 CLI tests on ANSI escapes.
- **`session_end` no longer loses or corrupts what it was given (#681, #682).** The project-status one-liner used to reference only `summary` — `decisions[]`/`blockers[]` were dropped with no trace — and cut the summary at 200 characters mid-word with no ellipsis, so the file read as though session-end had mangled its input. The status line now records how many decisions and blockers the session had plus a pointer to the indexed session-end file that holds them verbatim, keeps the file's one-line-per-session shape, and marks truncation with `...` on a word boundary at 400 characters. Separately, session-end now rejects (HTTP 400, with the offending fragment named) a `summary` or array entry carrying a leaked tool envelope or harness markup instead of embedding it as memory; detection requires a corroborating signal — absent arrays, an unmatched closing tag, or the fragment at the tail — and anything inside a code fence or backticks always passes, so notes *about* tool-call syntax stay saveable. The SessionEnd hook now strips Claude Code harness markup from its topic extraction at the source.
...
- **`recent_snapshots` session-start digest section (#668).** The `/context/prime` digest (`palinode_session_init` MCP tool, `POST /context/prime`, `palinode prime` CLI — ADR-010 parity) now surfaces the most recent `ProjectSnapshot`s for the resolved project — the `/wrap` "pick up where you left off" notes that previously were written by `palinode_session_end` but recallable only via `palinode_search`. Bounded to the 3 newest, newest-first, skipped when none, and inheriting the same scope (#107) and visibility (#108) filtering as every other digest section, so a private or off-chain snapshot never surfaces. Rendered first in the text digest as the single most useful resume note.
- **Visibility + private/restricted scopes — ADR-009 Layer 2 (#108).** Memory frontmatter gains a `visibility` axis (`inherited` | `private` | `restricted`) plus an `access` allowlist, enforced across every recall surface. **`private`** memories (agent scratch, reasoning traces) are visible only to their owning scope — a memory scoped `agent/researcher` with `visibility: private` is invisible to any other agent, so setting `PALINODE_AGENT` per process isolates each agent's scratch while shared `project/*` findings stay visible to all. **`restricted`** memories are gated solely by `access` intersecting the session's scope chain (an on-chain broad scope does not bypass the allowlist). Access control is advisory — enforced at the selection layer, not on disk (ADR-009 §3.4).
  Enforcement lives in **one choke point**, `palinode.core.visibility.is_visible`, that every recall surface routes through: `GET /list`, `POST /search` (both the semantic and the empty-query recency branch), `POST /search-associative`, and the `/context/prime` digest. Two properties make it one module rather than a filter per caller. It reads **live frontmatter, never `chunks.metadata`** — the indexer's unchanged-content fast path skips the chunk upsert when a file's section body is unchanged, so marking an existing memory private (a frontmatter-only edit, the canonical way since files are the source of truth) leaves the DB cache non-private indefinitely. And it **normalizes every path to memory-dir-relative** before any scope inference, since `chunks.file_path` is absolute while the digest scanner yields relative paths — a divergence that made the same root-level memory infer a real project scope on one surface and nothing on the next.
  **Access control is unconditional; scope isolation requires an identity.** `private`/`restricted` memories are withheld even from surfaces with no scope chain — notably `GET /list?core_only=true`, which the shipped SessionStart hook injects from, so a `core: true` private memory can never be auto-injected into every session. Scope filtering, by contrast, only engages when the chain carries a real identity level (`ScopeChain.has_identity`, which excludes the ephemeral `session` level): a bare ADR-007 `session_id` is recall telemetry, not an identity, and must not silently hide every explicitly-scoped shared memory. Classic-mode priming likewise still withholds access-controlled memories — classic is a *selection* mode, never a way around access control.
  **Absence-is-neutral holds end to end**: no `visibility` field = `inherited`, and `inherited` never consults the directory-inferred default scope, so legacy files are never hidden (the ADR-009 §7 zero-migration promise); malformed `visibility`/`access` values coerce to `inherited`/empty in `parse_scope`. Saves now **reject** `visibility: private` with no `scope:` and `visibility: restricted` with no `access:` (HTTP 400 with the fix in the message) — both declare a memory nobody can ever recall, including its author, and the failure belongs at write time rather than as a memory that silently disappears. Search filtering cannot be pushed into SQL (the authoritative frontmatter is in the file, not the row), so a starved result window is re-fetched wider **only when hidden rows actually consumed it** — leaving today's ranking and fetch limits byte-identical whenever nothing is hidden. Internal/maintenance callers (consolidation, dedup-suggest, `search_internal`) deliberately bypass the gate: they must see every memory and never return content to a session. New `palinode/core/visibility.py` + `tests/test_visibility_scopes.py` (46 tests: predicate semantics for all three visibilities incl. malformed values, `PALINODE_AGENT` cross-agent isolation, the unconditional `/list` gate, live-frontmatter-over-stale-cache, absolute-vs-relative path agreement, session-only chain non-activation, adaptive re-fetch, digest classic/scoped modes, and save-time rejection).
- **`bench/` — ingestion & recall benchmark harness (#650).** A dependency-light harness (Python stdlib + the `palinode` package only) that measures four properties of the memory pipeline on a fixed synthetic corpus: **cost per remembered fact** (ingest makes zero chat-LLM calls and one embed call per changed section, skipped on unchanged content via SHA-256 dedup), **determinism** (ingest the same corpus N≥5 times from clean state and byte-diff the resulting memory state — source files plus a normalized logical DB dump — target identical modulo timestamps), **LLM-free recall latency** (p50/p95 for hybrid and keyword-only search, cold and warm; query-time chat-model calls and query-vector embed calls are counted separately and never conflated — "LLM-free" means no chat/synthesis model, while hybrid still costs one embedding per query), and the **degradation floor** (recall with the embedder *actively disabled* → keyword-only, and with the API + index down → grep over the source files; recall never reaches zero because files are the source of truth). The corpus generator is a pure function of `(seed, size)` producing byte-identical fixture material (`my-project`/`other-project`, Alice/Bob) — no real memories. The harness auto-detects a reachable embedder and measures the hybrid path when present, otherwise degrades to keyword-only and reports those numbers honestly. Runnable end-to-end (`python -m bench.run --out results.json`; `python -m bench.report results.json`). New `tests/test_bench_harness.py` (corpus determinism, embedded + keyword-only ingest smoke, state-fingerprint stability, keyword/grep recall, report generation, and the subprocess determinism axis).
- **`palinode trace <file>` — the provenance composition command (#536, v0.9.4 headline).** One command joins every provenance primitive Palinode already exposes into a single lineage view for a memory file: the source-citation anchors it carries (G1), the commit that first **saved** it and the commits that **changed** it (git blame/history), the supersession trail (the executor's `-history.md` sibling plus in-body `[superseded]`/`[retracted]` tombstones), the typed `contradicts` / `backed_by` links (G4), and how often it has been **recalled** (the retrieval log, with sessions + dates). Each row carries an honest three-state status — `present` (data exists), `none` (the capability landed but this file has nothing for it), or `not_captured` (the provenance gap itself isn't built yet) — so the trail never overclaims; the two rows still gated on unbuilt gaps, **Extracted** (per-fact extraction metadata, G2 #534) and **Used in** (the terminal consuming-action edge, G3 #535), render a muted "not yet captured" placeholder and will fill in without a shape change as those gaps land (rows name the gap by its stable public label — `G2`/`G3` — never by a tracker issue number, which would resolve to something unrelated in the public package). TTY-aware per ADR-010: a human-readable vertical lineage when interactive, and a structured JSON object when piped — the object the local review UI (C2) consumes at `GET /trace/{file}`. Lands on three surfaces — CLI `palinode trace`, **`GET /trace/{file_path}`**, and the `palinode_trace` MCP tool (full-surface, read-only) — registered in `palinode/core/parity.py` (plugin-exempt like its blame/rollback/history siblings); MCP schema + API route-inventory goldens regenerated and the MCP-SETUP tool table updated. New `palinode/core/trace.py` (composition engine + shared text renderer, `git_tools.first_commit` provenance primitive) and `tests/test_trace_536.py` (26 tests: saved/changed split, seeded supersession chain, source anchors, typed links, recall aggregation incl. non-canonical logged paths, honest placeholders, degraded inputs (malformed frontmatter, directory path), REST endpoint incl. 404 + traversal rejection, TTY-aware CLI, MCP tool surface + dispatch).
- **Fresh-session context validation, end-to-end — closes the cross-session test gap (#186).** The whole ADR-012 session-start surface shipped in v0.9.3 (hook, `POST /context/prime`, `palinode_session_init`, `palinode prime`), but coverage exercised each surface in isolation; nothing proved the *cold-start sequence whole* — a fresh client with a populated store getting the right context, failure modes visible not silent. New `tests/test_fresh_session_context.py` (10 integration-style tests, real frontmatter files + real SQLite for the save path) simulates that sequence: (a) the hook-shaped `{cwd, session_id}` POST returns the resolved project's digest and **never bleeds the distractor project** (asserted both directions); (b) the same digest reaches `palinode_session_init` (with `mcp._post` routed through the real endpoint + store, closing the loop the 262 unit test stubbed) and the CLI renderer `format_context_digest`; (c) degraded paths stay sane and *labelled* — no resolvable project → core-only, empty store → a coherent "nothing yet" digest, malformed/non-mapping/wrong-typed frontmatter files are skipped without crashing the scan; (d) cross-session persistence — memories written through the real `/save` path are visible to a genuinely fresh app/client instance and stay scope-isolated, proving the on-disk memory dir is the only channel between sessions. No product defect surfaced (the digest core, scope filter, and frontmatter scan are already robust); one design boundary noted for follow-up — session-end `ProjectSnapshot`s (the `/wrap` artifact) are search-only, not part of the prime digest's three sections.
- **`POST /context/prime` is scope-chain aware — ADR-009 Layer 1 slice 4 (#107).** The session-start priming endpoint (live since v0.9.3, #262) now resolves the session's scope chain server-side and honors ADR-009 §3.5 request overrides: optional `mode` (`classic` | `scoped`; `smart` stays Layer 3 and is rejected 422) and an explicit `scope` object that replaces resolution entirely. The response keeps the shipped ADR-012 digest contract — `{project, core_memories, recent_decisions, open_action_items, _palinode_hint}` — extended with `mode` and `scope_chain`; in `scoped` mode every digest section drops memories whose **explicit** `scope:` frontmatter is off the chain (unscoped memories always pass — the ADR-009 §7 promise, slice 3's `chain_allows` predicate threaded through the digest core). Project resolution gains `PALINODE_PROJECT` env precedence (explicit arg → env → `project_map` → basename auto-detect — now matching the MCP ambient-context order), and **`scope.prime_mode` defaults to `scoped`** — the flip slice 1 deferred until slices 2–3 landed; safe because scoped is behavior-identical to classic until someone actually writes explicitly-scoped memories. The hook contract stays frozen: bare `{cwd, session_id}` (or `{}`) payloads succeed unchanged. MCP `palinode_session_init` and CLI `palinode prime` both flow through this endpoint, so they inherit scoped selection with no surface changes — slice 5 verified this and closed out Layer 1: four-surface parity registered as `context_prime` (plugin exempt), SessionStart-hook comments updated to drop the pre-endpoint "harmless 404" language (both drift-guarded copies), ADR-009 flipped Proposed → Accepted. Reconciles the parallel slice-4 branch (PR #640) with the shipped #262 endpoint. New `tests/test_context_prime.py` (13 tests: frozen hook contract, mode semantics + fallbacks, override chain, resolution precedence, digest-shape rows, section-wide filtering).

### Changed

- **Light `/wrap` gains a Step 0 git pre-flight — offer to land open working-repo work before archiving (#618).** The light `WRAP_COMMAND_BODY` (the default `/wrap`) was Palinode-only: it pushed the memory repo and archived the session but never looked at the *working* repo's git state, so a session ending on a dirty tree or a feature branch ahead of `main` got a "safe to /clear now" that implied more closure than actually happened. A new **Step 0** runs ahead of the existing push → session_end steps: it checks `git status --short` and whether the branch is ahead of `main`, and if either is non-empty it states the situation in one line and **offers** to close it out first (commit + ff-only merge; push only when explicitly asked, honoring the repo's push policy). It is offer-only — it never commits, merges, or pushes without an explicit in-session yes, and declining proceeds to the archive unchanged, so the light variant keeps its non-mutating contract and doesn't duplicate the opt-in heavy wrap's halt-on-failure merge sequence. Single-source: Step 0 is added once to the constant, so both the scaffolded `.claude/commands/wrap.md` and the rendered `.claude/skills/wrap/SKILL.md` inherit it (no drift). The heavy variant is untouched — it already lands work with halt semantics, making a courtesy offer redundant. New tests in `tests/test_cli_init.py`, `tests/test_init_wrap_heavy.py`, and `tests/test_init_skills.py` (Step 0 present in the light body/command/skill, absent from heavy, offer-not-force wording, clean/dirty/branch-ahead paths).

### Fixed

- **Session-end hook: the manual-install copy no longer silently drifts from what `palinode init` installs (#633).** The SessionEnd hook shipped in two copies — the embedded `HOOK_SCRIPT` in `palinode/cli/init.py` (what `palinode init` installs) and `examples/hooks/palinode-session-end.sh` (what the README's "Manual install" path copies) — and they had functionally diverged: the embedded copy carried the **/wrap-skip dedup gate** (skip the deterministic floor capture when the transcript shows `palinode_session_end` already ran, `PALINODE_HOOK_FORCE=1` to override), the **fallback-log-on-POST-failure** path (`.claude/session-floor-fallback.jsonl`, so a capture is never lost when the API is down), and **`PALINODE_HOOK_DRYRUN`**, while the manual copy had none of them — a manual-install user got duplicate captures after `/wrap`, silently dropped captures on API failure, and no dry-run. The richer body is now promoted to the canonical `examples/hooks/palinode-session-end.sh`, the embedded constant is a **byte-for-byte** mirror of it, and a drift-guard test (`test_hook_script_byte_identical_to_canonical`, mirroring the session-start guard) pins the two so they can't re-drift. While reconciling, ported the session-start hook's **`PALINODE_API_TOKEN` bearer-auth** support to session-end for parity on token-protected deployments. New tests in `tests/test_session_end_timeout.py` (byte-identity guard) and `tests/test_cli_init.py` (bearer header present when configured, absent by default).
- **`recall_count` no longer double-counts on the visibility-widened search path (#667, follow-up to #660).** The ADR-009 Layer 2 visibility gate re-fetches a wider FTS/vector window when hidden (private/restricted/off-chain) rows starved the original one; because each store fetch records ADR-006/007 recall metadata for its own hit set, a row present in **both** the initial and widened passes had its `recall_count` incremented twice for one logical retrieval — inflating the recall signal (`recall_count` / `last_recalled`) that the learning loop consumes, for exactly the memories that trip widening. `_fetch_visible` now de-duplicates by chunk id across the two passes: the widened re-fetch runs with store-level recall recording suppressed (`store.search` / `store.search_hybrid` gain a `record_access=False` path), and access is recorded once for only the rows the wider window newly surfaced. Ranking, fetch limits, and the #660 visibility semantics are byte-identical whenever widening does not fire; the per-session importance nudge (already `session_id`-deduped) and the retrieval log (written once, on the final set) are unaffected. New regression in `tests/test_visibility_scopes.py` asserts a single retrieval that trips widening increments `recall_count` exactly once for a row in both passes, and is non-vacuous — it asserts the wider re-fetch actually fired.
- **Plugin parity for `claims` and `unverified` restored at source (#646).** The v0.9.3 release registered claim-level source anchors (#508) and the `unverified` epistemic value (#589) as four-surface capabilities, but the OpenClaw plugin's TypeScript half never landed on dev: `plugin/index.ts` exposed neither the `claims` parameter nor the `unverified` literal. The public v0.9.3 sync's exact-payload gate caught it (`npm test`: 1 failure) and the payload was corrected in-flight; this ports that exact fix back to source — structured `claims` TypeBox schema (`claim_id`, `text`, `source_id`, `span{quote, quote_hash}`, `anchor_id`), `unverified` in the epistemic union, and the three regression tests (schema presence ×2, request-body forwarding) — so v0.9.4 can't overwrite the public fix. The gap existed because the plugin's vitest suite wasn't wired into dev CI; a new `plugin-tests` job now runs `npm test` on every PR and in the post-merge main sweep. Also upgrades the plugin's dev-only test stack to Vitest 4 (drop-in; 61 tests green), clearing all 5 `npm audit` advisories rooted in the old esbuild/vite chain.
...
- **Source-quote integrity primitive (`palinode.core.quote_verify`, #459 / public #65 Q2).** The deterministic, offline core of the quote-drift check requested upstream: `normalize_quote` / `quote_hash` (smart-punctuation + whitespace folding for stable hashing), `verify_quote` (pure — distinguishes an internally-tampered anchor from a drifted source), and `verify_memory_sources` (verifies a file's proposed `sources:` anchors, a clean no-op when none are present). Intentionally a plain integrity check — no cryptographic signing or attestation — so it has no dependency on the separate memory-attestation work. The frontmatter capture schema and the `doctor`/`lint` surface are outside this change; this lands the verifier mechanism with full unit coverage (`tests/test_quote_verify.py`).
